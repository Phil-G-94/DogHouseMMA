...
const ShopCartButton = ({ showCart }) => {
    ...
    const numberOfShopCartItems = shopCartCtx.items.reduce((currentValue, item) => {
        return currentValue + item.quantity;
    }, 0);

    return (
        <button
            className="flex flex-row place-items-start gap-2 bg-amber-500 rounded-md p-2 text-black"
            onClick={showCart}
        >
            <span>Cart: </span>
